...
`--with-deps` is the only step that installs anything. It fetches `superpowers`
from the official Claude plugin marketplace and `gsd-core` from npm — nothing
else, unless you ask. A third-party source (`obra/superpowers-marketplace`) also
...
- `summarise-current-tasks` — the approval view of a plan: two lines per task in
  the plan's own numbering, marking only what is irreversible, secret-touching
  or read-only, so the work can be signed off before it starts.
